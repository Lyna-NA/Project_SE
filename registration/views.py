...
@allowedUsers('customer')
def resetInfo(request ,username):
    if request.method =="GET":
        context = {
            'customer' :get_object_or_404(Customer,username=username),
        
        }
        return render (request , 'registration/auth/resetInfo.html',context)
    if request.method =="POST":
        ...
        c =get_object_or_404(Customer,username=username)
        ...
        c.state=state
        c.street=street
        c.building_num= building_num
        c.dept_num=dept_num
        c.phones=phones
        c.save()
        print(c.username,c.user,c.gender)
        messages.add_message(request,messages.SUCCESS,'Added successfully')
